...
此分頁不使用本機快取結果，會直接透過 API 抓股票清單與日線資料。後端回傳 `data_policy=direct_api_only`，不讀取也不寫入本機 SQLite 市場資料。`資料失敗` 代表 API 取不到、日線資料不足或解析失敗；`未符合` 代表已成功取得資料，但 KDJ / MACD / 布林條件不成立。`掃描上限` 選 `全部` 時會逐檔抓取全部 API 股票，時間會比較長。
