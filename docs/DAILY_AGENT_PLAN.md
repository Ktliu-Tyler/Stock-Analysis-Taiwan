...
- 日線型態標籤：標示「MACD空頭減弱 + KDJ即將金叉」等轉折預備訊號。
...
新增「MACD空頭減弱 + KDJ將金叉」日線型態，目標是找出空方力道正在收斂、但 KDJ 尚未正式黃金交叉的股票。

判斷條件：

- MACD histogram 仍小於 0，代表還在空方區。
- 最近 4 根日線 histogram 至少有 2 次往上改善，且最新值高於該段第一根。
- K 值仍低於 D 值，尚未完成黃金交叉。
- K/D 差距大於 0 且小於等於 6，並且比前一根日線縮小。
- K 值上升，J 值上升或 J 值已高於 D 值。
- K、D 未進入過熱區，避免追到高檔鈍化訊號。
...
- 完整成立時，分數模型增加日線轉折預備加分，並在推薦理由中標示。
- 只成立部分條件時，保留低權重提示，方便人工追蹤但不視為完整型態。
- 晨報與海報後續可把此旗標納入 Top picks 標籤，讓手機閱讀時先看到關鍵技術型態。
